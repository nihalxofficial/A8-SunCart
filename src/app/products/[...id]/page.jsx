                ...
                <div className="bg-white rounded-2xl shadow-lg border border-sky-100 overflow-hidden mb-10">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 p-6 md:p-8">
                        ...
                        <div className="relative">
                            <div className="relative h-80 md:h-96 lg:h-112.5 bg-linear-to-br from-gray-50 to-amber-50 rounded-xl overflow-hidden">
                                <Image
                                    src={product.image}
                                    alt={product.name}
                                    fill
                                    className="object-cover border-2 border-gray-100 shadow"
                                    priority
                                />
                            </div>
                            ...
                            <div className="absolute top-4 right-4 bg-linear-to-r from-amber-500 to-orange-500 text-white text-xs font-bold px-3 py-1 rounded-full shadow-md">
                                -50% OFF
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col">
                            ...
                            <div className="mb-4">
                                <div className="flex items-baseline gap-3">
                                    ...
                                    <span className="text-lg text-stone-400 line-through">${(product.price * 2).toFixed(2)}</span>
                            ...
                            <div className="flex flex-col sm:flex-row gap-3 mb-6">
                                <button className="flex-1 cursor-pointer bg-linear-to-r from-sky-500 to-cyan-400 hover:from-sky-600 hover:to-cyan-500 text-white font-semibold py-3 rounded-xl transition shadow-md hover:shadow-lg flex items-center justify-center gap-2">
                                    <ShoppingCart size={18} />
                                    Add to Cart
                                </button>
                                <button className="flex-1 cursor-pointer border-2 border-sky-500 text-sky-600 hover:bg-sky-50 font-semibold py-3 rounded-xl transition flex items-center justify-center gap-2">
                                    <Heart size={18} />
                                    Wishlist
                                </button>
                                <button className="p-3 border cursor-pointer border-stone-200 rounded-xl hover:bg-amber-50 hover:border-amber-200 transition">
                            ...
                            {relatedProducts.map((related) => (
                                <Link key={related.id} href={`/products/${related.id}`} className="group">
                                    <div className="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 border border-sky-100 hover:border-amber-200">
                                        <div className="relative h-40 bg-linear-to-br from-gray-50 to-amber-50">
                                            <Image
                                                src={related.image}
                                                alt={related.name}
                                                fill
                                        ...
                                        <div className="p-3">
                                            ...
                                            <div className="flex items-center gap-1.5">
                                                ...
                                                <span className="text-[10px] text-stone-400 line-through">${(related.price * 2).toFixed(2)}</span>
                                            </div>
                                        </div>
                                    </div>
                                </Link>
